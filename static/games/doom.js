...
(function () {
  ...
  var LEVELS = [
    ...
    {
      name: "ЗОНА 4 · КАНАЛИЗАЦИЯ",
      sky: [18, 26, 32], floor: [30, 36, 34],
      map: [
        "############################",
        "#p...=====........=====....#",
        "#.h..=...=..zz....=...=..a.#",
  ...
  function start(root, api) {
    ...
    function drawAutomap() {
      ...
      for (var y = 0; y < state.grid.length; y++) {
        for (var x = 0; x < state.grid[y].length; x++) {
          ...
          if (!c) continue;
          ctx.fillStyle = c.exit ? "#5cff9d"
            : c.key ? ({ red: "#d63a3a", blue: "#4472e8", yellow: "#e0c020" })[c.key]
            : c.door ? "#8a6a3a" : "#2f5f73";
          ctx.fillRect(ox + x * cell, oy + y * cell, cell - 1, cell - 1);
          // Выход подсвечиваем и обводим: раньше он был почти того же
          // жёлтого, что и запертая дверь, и их путали между собой.
          if (c.exit) {
            ctx.strokeStyle = "rgba(92,255,157,.9)";
            ctx.lineWidth = 2;
            ctx.strokeRect(ox + x * cell - 2, oy + y * cell - 2, cell + 3, cell + 3);
          }
        }
      }
    ...
    if (api.touch) {
      ...
      api.deck({
        ...
        stickBig: true,
        airy: true,          // огонь чуть подальше от ручки
        actions: [
