...
- **Настройка SSH-консоли после первого деплоя с этой фичей** — нужен только один шаг: создать на сервере `/opt/sites/vitazgio.ru/.env` с `SSH_GATE_PASSWORD_PREFIX=<свой секрет>` (не коммитить) и пересоздать контейнер (`docker compose up -d --build`). Логин/пароль к самим устройствам никуда заранее прописывать не нужно — их спрашивают в браузере при каждом подключении.
