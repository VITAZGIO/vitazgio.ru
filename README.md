...
- **Настройка SSH-консоли после первого деплоя с этой фичей:**
  1. Вписать реальные `ssh_user` вместо `CHANGE_ME` у трёх устройств в `NETBIRD_DEVICES` (app.py) и задеплоить.
  2. Создать на сервере `/opt/sites/vitazgio.ru/.env` с `SSH_GATE_PASSWORD_PREFIX=<свой секрет>` (не коммитить).
  3. После рестарта контейнера посмотреть `docker logs vitazgio-site` — там будет напечатан сгенерированный публичный SSH-ключ, его нужно добавить в `~/.ssh/authorized_keys` на orangepizero3, ubuntu-server и ubuntuvitaz1.
