 ...
 * (`MainActivity`), а токен приезжает с сайта сам. Экран остался потому, что
 * без журнала удалённая отладка превращается в гадание: открывается долгим
 * тычком по полоске состояния поверх сайта.
 */
